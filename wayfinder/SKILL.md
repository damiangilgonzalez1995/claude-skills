...
- **Research** (AFK): Leer documentación, APIs de terceros, o recursos locales como bases de conocimiento para sacar a la luz un hecho del que depende una decisión. Se resuelve con un **subagente** `/deep-research`. Úsalo cuando se requiere conocimiento fuera del directorio de trabajo actual.
...
5. **Dispara los subagentes de research.** Por cada ticket `research` que acabas de crear, levanta un subagente `/deep-research` para resolverlo en paralelo, capturando sus hallazgos en una rama desechable `research/<nombre>` con un puntero de contexto desde el ticket.
